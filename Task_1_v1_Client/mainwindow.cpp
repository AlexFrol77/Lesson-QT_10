    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(client, &TCPclient::sig_connectStatus, this, &MainWindow::DisplayError);
...
{
    ui->tb_result->append("Текущие время: " + time.toString());
}
void MainWindow::DisplayFreeSpace(uint32_t freeSpace)
{
    ui->tb_result->append("Свободное мсто на сервере: " + QString::number(freeSpace));
}
void MainWindow::SetDataReply(QString replyFreeSpace)
{
    ui->tb_result->append("Введенные данные: " + replyFreeSpace);
}
void MainWindow::DisplayStat(StatServer stat)
{
    ui->tb_result->append("Полученно байт: " + QString::number(stat.incBytes));
    ui->tb_result->append("Отправленно байт: " + QString::number(stat.sendBytes));
    ui->tb_result->append("Полученно пакетов: " + QString::number(stat.revPck));
    ui->tb_result->append("Отправленно пакетов: " + QString::number(stat.sendPck));
    ui->tb_result->append("Время соединения: " + QString::number(stat.workTime));
...
void MainWindow::DisplayError(uint16_t error)
{
    ...
    case ERR_NO_FREE_SPACE: {
        QMessageBox msg;
        msg.setText("Недостаточно свободного места");
        msg.exec();
        break;
    }
    case ERR_NO_FUNCT: {
        QMessageBox msg;
        msg.setText("Функционал не реализован");
        msg.exec();
        break;
    }
...
void MainWindow::DisplaySuccess(uint16_t typeMess)
{
    switch (typeMess) {
        case CLEAR_DATA: {
            ui->tb_result->setText("Память очищена!");
            break;
        }
        default:
            ui->tb_result->setText("Что то пошло не так!");
            return;
    }
}
   ...
   switch (ui->cb_request->currentIndex()){
       ...
       //Отправить данные
        case 3: {
            ...
            QString strData = ui->le_data->text();
            ui->le_data->clear();
            header.len = sizeof(strData);
            client->SendData(header, strData);
            break;
        }
